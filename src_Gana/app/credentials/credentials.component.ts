...
//         });
        ...
        this.AddNewTask = this.fb.group({
          ...
          key :['', [Validators.required,Validators.minLength(3),Validators.maxLength(52)]],
          passPhrase:['',[Validators.required,Validators.minLength(3),Validators.maxLength(10)]],
          type:['',[Validators.required,Validators.minLength(3),Validators.maxLength(52)]]
...
this.testvalue=value.credentialId;
...
CredentailNameUpdate;
keyUpdate;
passPraseUpdate;
typeupdate;
defaultUpdateValues(){
  this.CredentailNameUpdate=this.DetailSelected.name;
  this.keyUpdate=this.DetailSelected.key;
  this.passPraseUpdate=this.DetailSelected.passphrase;
  this.typeupdate=this.DetailSelected.type;
  this.statusupdate=this.DetailSelected.isActive;

  console.log("this.DetailSelected.isactive--->"+this.DetailSelected.isActive);
  ...
  this.CredentialService.getCredentials().subscribe(product => {
   ...
   this.BusinessGroupCrudService.clientDetails=product.json().credentials;
  this.persons=product.json().credentials;
  this.clientCounter=product.json().credentials.length;
  console.log("Total Records---->"+this.clientCounter);

            for(let i=0;i<=product.json().credentials.length-1;i++){
            if(i==0){
              this.DetailSelected=product.json().credentials[i];
              console.log("this is credentials"+this.DetailSelected)
              this.testvalue=this.DetailSelected.credentialId;
              this.defaultUpdateValues();
               this.DataToCsv=product.json().credentials[i];
...
console.log(this.CredentailNameUpdate);
//console.log(this.clientNameupdate);
console.log(this.keyUpdate);
console.log(this.passPraseUpdate);
...
let credentialsInfo =
{ 
"credentialId":this.DetailSelected.credentialId,
"name":this.CredentailNameUpdate,
      "key": this.keyUpdate,
      "passPhrase": this.passPraseUpdate,
      "type": this.typeupdate,
      "isActive": isactive
      
}
let ClientInfotwo=JSON.stringify(credentialsInfo);
...
//let url="https://10.31.15.210:8243/ACP/1.0/acp/clients/"+this.DetailSelected.clientId
 let url="http://10.31.15.210:9000/acp/credentials/"+this.DetailSelected.credentialId
console.log(url);
this.CredentialService.UpdateCredentialDetails(ClientInfotwo,url).subscribe(product => {
// this.persons=product.json().clients;

  console.log("after update"+product.json());
  this.DetailSelected=product.json().credentialsInfo;
  this.IncidentEditable=true;
 console.log(product.status);
...
// this.ClientCrudService.statuscode=product.status;
  ...
  deleteClient(){
    ...
    this.CredentialService.DeleteCredentialDetails(this.DetailSelected.credentialId).subscribe(product => {
...
//  temppostvalues.status='active';
  ...
  this.CredentialService.postCredentialDetails(JSON.stringify(temppostvalues)).subscribe(product => {
  ...
  MakeAddClientEnable(){
    ...
    this.SharedService.OptionSelectedtest.next('ADD NEW CREDENTIAL');
...
ClientUpdateValidation():boolean{
  if(!this.IncidentEditable){
       if(this.CredentailNameUpdate==''){
         this.clientUpdateErrorMessage="Credential Name Cant be empty"
         return true;
       }
       else if(!(/^[a-zA-Z\s]+$/.test(this.CredentailNameUpdate))){
        this.clientUpdateErrorMessage="Credential Name Cant have numbers or any spectialcharecters"
        return true;
       }
       else if(this.CredentailNameUpdate.length<3){
        this.clientUpdateErrorMessage="Credential Name Cant be less than 3 charecters"
        return true;
       }
       else if(this.CredentailNameUpdate.length>40){
        this.clientUpdateErrorMessage="Credential Name Cant be greater than 40 charecters"
        return true;
...
shortNameUpdateValidation(){
  if(!this.IncidentEditable){
    if(this.keyUpdate==''){
      this.shortNameUpdateErrorMessage="Key Cant be empty"
      return true;
    }
    else if(!(/^[a-zA-Z]+$/.test(this.keyUpdate))){
     this.shortNameUpdateErrorMessage="Key Cant have numbers or any spectialcharecters"
     return true;
    }
    else if(this.keyUpdate.length<3){
     this.shortNameUpdateErrorMessage="Key Cant be less than 3 charecters"
     return true;
    }
    else if(this.keyUpdate.length>40){
     this.shortNameUpdateErrorMessage="Key Cant be greater than 40 charecters"
     return true;
...
addressupdateValidation(){
  if(!this.IncidentEditable){
    if(this.passPraseUpdate==''){
      this.addressupdateErrorMessage="PASS PHRASE Cant be empty"
      return true;
    }
    else if(!(/^[a-zA-Z0-9]+$/.test(this.passPraseUpdate))){
     this.addressupdateErrorMessage="PASS PHRASE Cant have any spectialcharecters"
     return true;
    }
    // else if(this.addressupdate.length<3){
    //  this.addressupdateErrorMessage="address Cant be less than 3 charecters"
    //  return true;
    // }
    // else if(this.addressupdate.length>40){
...
remarksupdateValidation(){
  if(!this.IncidentEditable){
    if(this.typeupdate==''){
      this.remarksupdateErrorMessage="TYPE Cant be empty"
      return true;
    }
    else if(!(/^[a-zA-Z]+$/.test(this.typeupdate))){
     this.remarksupdateErrorMessage="TYPE Name Cant have any spectialcharecters"
     return true;
    }
