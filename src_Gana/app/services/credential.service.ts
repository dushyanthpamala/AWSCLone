...
export class CredentialService {
  ...
  getCredentials(){
    ...
    return this.http.get('http://10.31.15.210:9000/acp/credentials',options).map((response) => response);
  ...
  postCredentialDetails(valaues){
    ...
    let options = new RequestOptions({ headers: headers });
    return this.http.post('http://10.31.15.210:9000/acp/credentials',valaues,options).map((response) => response);
    
    }
    


    DeleteCredentialDetails(subscriberID){

      let url='http://10.31.15.210:9000/acp/credentials/'+subscriberID;
